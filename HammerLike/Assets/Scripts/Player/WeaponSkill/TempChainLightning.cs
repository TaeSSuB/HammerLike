...
public class TempChainLightning : MonoBehaviour, ISkill
{
    ...
    {
        Vector3 thunderPosition = ClampPositionToMaxDistance(position);
    ...
    IEnumerator DoLightning(Vector3 startPosition)
    {
        Vector3 startLocation = startPosition;
        Quaternion quater = transform.rotation * Quaternion.Euler(90, 0, 0);

        GameObject attack = Instantiate(Lightning, startLocation, quater);
        Vector3 previousLocation = startLocation;

        GameObject firstTarget = FindNearestMonster(startLocation);
        if (firstTarget != null)
        {
            Vector3 targetPoint = firstTarget.transform.position;
    ...
    public void WeaponAttack(Vector3 position)
    {
        ...
        StartCoroutine(DoLightning(position));
    }
