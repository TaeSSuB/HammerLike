...
public class B_Pillar : B_Prop
{
    ...
    public void DestroyMesh()
    {
        ...
        var parts = new List<B_PartMesh>();
        var subParts = new List<B_PartMesh>();

        var mainPart = new B_PartMesh()
        {
            UV = originalMesh.uv,
        ...
        for (var c = 0; c < CutCascades; c++)
        {
            ...
            parts = new List<B_PartMesh>(subParts);
            subParts.Clear();
        }
    ...
    private B_PartMesh GenerateMesh(B_PartMesh original, Plane plane, bool left)
    {
        var partMesh = new B_PartMesh() { };
        var ray1 = new Ray();
        var ray2 = new Ray();
    ...
    private void AddEdge(int subMesh, B_PartMesh partMesh, Vector3 normal, Vector3 vertex1, Vector3 vertex2, Vector2 uv1, Vector2 uv2)
    {
        if (!edgeSet)
        {
            edgeSet = true;
...
public class B_PartMesh
{
    ...
    /// <summary>
    /// 명진.0515
    /// 깰 수 있는 문 오브젝트를 만들었는데 현재는 오버로딩으로 대체했는데 템플릿으로 변경예정
    /// </summary>
    /// <param name="original"></param>
    public void MakeGameobject(B_Pillar original)
    {
    ...
    public void MakeGameobject(B_BreakableDoor original)
    {
        GameObject = new GameObject(original.name);
        GameObject.transform.position = original.transform.position;
        GameObject.transform.rotation = original.transform.rotation;
    ...
    public void MakeGameobject(B_Rock original)
    {
        GameObject = new GameObject(original.name);
        GameObject.transform.position = original.transform.position;
        GameObject.transform.rotation = original.transform.rotation;
