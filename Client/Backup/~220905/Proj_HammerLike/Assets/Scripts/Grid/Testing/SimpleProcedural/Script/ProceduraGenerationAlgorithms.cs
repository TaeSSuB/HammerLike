...
public class ProceduraGenerationAlgorithms
{
    // HashSet - 중복을 허용하지 않는 무작위 구현 클래스
    public static HashSet<Vector2Int> SimpleRandomWalk(Vector2Int startPosition, int walkLength)
    {
        ...
        {
            var newPosition = previousPosition + Direction2D.GetRndCardinalDir();
    ...
    // 입출력 순서를 관리하기 위해 List 사용
    public static List<Vector2Int> RandomWalkCorridor(Vector2Int startPosition, int corridorLength)
    {
        List<Vector2Int> corridor = new List<Vector2Int>();
        var direction = Direction2D.GetRndCardinalDir();
        var currentPosition = startPosition;
        corridor.Add(currentPosition); // 시작지점 추가
...
public static class Direction2D
{
    public static List<Vector2Int> cardinalDirList = new List<Vector2Int>
    {
        ...
        new Vector2Int(-1, 0), // A
        new Vector2Int(0, -1), // S
        new Vector2Int(1, 0), // D
    };

    public static Vector2Int GetRndCardinalDir()
    {
