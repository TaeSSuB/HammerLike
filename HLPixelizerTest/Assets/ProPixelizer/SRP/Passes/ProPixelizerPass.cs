...
#pragma warning disable 0618
        ...
        public RTHandle PixelatedColorTarget(ref RenderingData data, ProPixelizerLowResolutionTargetPass target) =>
            Settings.TargetForPixelation == TargetForPixelation.CameraTarget
            ? ColorTarget(ref data)
            : target.Color;

        public RTHandle PixelatedDepthTarget(ref RenderingData data, ProPixelizerLowResolutionTargetPass target) =>
            Settings.TargetForPixelation == TargetForPixelation.CameraTarget
...
#if UNITY_2023_3_OR_NEWER
        protected static TextureHandle CameraColorTexture(UniversalResourceData resource, UniversalCameraData camera) => camera.cameraType != CameraType.Preview ? resource.cameraColor : resource.backBufferColor;
        protected static TextureHandle CameraDepthTexture(UniversalResourceData resource, UniversalCameraData camera) => camera.cameraType != CameraType.Preview ? resource.cameraDepth : resource.backBufferDepth;
#endif
